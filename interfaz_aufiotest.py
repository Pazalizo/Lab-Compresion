...
class InterfazCompresionAudioTest:
    """
    ...
    def __init__(self, root):
        ...
        # Botón para graficar
        self.boton_graficar = tk.Button(root, text="Mostrar Gráfica", command=self.graficar_audio)
        self.boton_graficar.grid(row=4, column=2, columnspan=3, pady=10, padx=10, sticky="nswe")
    ...
    def comprimir_audio(self):
        ...
        if self.audio_grabado is not None:
            try:
                ...
                self.enable_buttons()
                self.boton_graficar.config(state=tk.NORMAL)  # Activar botón de graficar
    ...
    def graficar_audio(self):
        """
        Método para graficar el audio original y el comprimido.
        """
        if self.audio_grabado is None or self.audio_comprimido is None:
            messagebox.showwarning("Advertencia", "Debe grabar y comprimir el audio antes de graficarlo.")
            return
        ...
        plt.subplot(2, 1, 1)
        plt.plot(self.audio_grabado, color="blue")
        plt.title("Audio Original")
        plt.xlabel("Muestras")
        plt.ylabel("Amplitud")

        # Graficar audio comprimido
        plt.subplot(2, 1, 2)
        plt.plot(self.audio_comprimido, color="orange")
        plt.title("Audio Comprimido")
        plt.xlabel("Muestras")
        plt.ylabel("Amplitud")
